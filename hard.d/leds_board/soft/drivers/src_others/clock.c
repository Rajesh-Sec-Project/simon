 ...
 */
#ifdef STM32F10X_CL
//#warning "La configuration d'horloge sur la famille Connectivity Line n'a jamais été testé en vrai: A VOS RISQUES ET
//PERILS !!!"
#endif
/*
...
#if((_PLLMUL != 4) && (_PLLMUL != 5) && (_PLLMUL != 6) && (_PLLMUL != 7) && (_PLLMUL != 8) && (_PLLMUL != 9) && \
    (_PLLMUL != _PLLMUL_6_5))
#error "_PLLMUL value invalid! Should be either 4,5,6,7,8,9 or _PLLMUL_6_5"
#endif
#endif

#if((_HPRE != 1) && (_HPRE != 2) && (_HPRE != 4) && (_HPRE != 8) && (_HPRE != 16) && (_HPRE != 64) && \
    (_HPRE != 128) && (_HPRE != 256) && (_HPRE != 512))
...
#define __RCC_CFGR_VALUE                                                                                        \
    (RCC_PLLMUL_VAL(_PLLMUL) | _PLLXTPRE_MASK | RCC_PLLSRC_IS_HSE | RCC_ADCPRE_VAL(_ADCPRE) | RCC_CFGR_SW_HSI | \
     RCC_PPRE1_VAL(_PPRE1) | RCC_PPRE2_VAL(_PPRE2))
#define HSE_IS_USED
#define PLL_IS_USED
#elif((_SYSCLK_SOURCE == IS_PLL) && (_PLL_SOURCE == IS_HSI))
#define __RCC_CFGR_VALUE                                                                                        \
    (RCC_PLLMUL_VAL(_PLLMUL) | _PLLXTPRE_MASK | RCC_PLLSRC_IS_HSI | RCC_ADCPRE_VAL(_ADCPRE) | RCC_CFGR_SW_HSI | \
     RCC_PPRE1_VAL(_PPRE1) | RCC_PPRE2_VAL(_PPRE2))
#define HSI_IS_USED
#define PLL_IS_USED
#elif(_SYSCLK_SOURCE == IS_HSI)
#define __RCC_CFGR_VALUE (RCC_ADCPRE_VAL(_ADCPRE) | RCC_CFGR_SW_HSI | RCC_PPRE1_VAL(_PPRE1) | RCC_PPRE2_VAL(_PPRE2))
#define HSI_IS_USED
#elif(_SYSCLK_SOURCE == IS_HSE)
#define __RCC_CFGR_VALUE (RCC_ADCPRE_VAL(_ADCPRE) | RCC_CFGR_SW_HSI | RCC_PPRE1_VAL(_PPRE1) | RCC_PPRE2_VAL(_PPRE2))
#define HSE_IS_USED
...
#define __RCC_CFGR_VALUE                                                                                               \
    (RCC_PLLMUL_VAL(_PLLMUL) | RCC_PLLSRC_IS_HSE | RCC_ADCPRE_VAL(_ADCPRE) | RCC_CFGR_SW_HSI | RCC_PPRE1_VAL(_PPRE1) | \
     RCC_PPRE2_VAL(_PPRE2))

// Modif Thierry : decalages -1 sur _PREDIV_1 et 2 et - 2 sur _PLL2MUL
#define __RCC_CFGR2_VALUE_TMP ((_PREDIV_1 - 1) | ((_PREDIV_2 - 1) << 4) | ((_PLL2MUL - 2) << 8))
...
#elif((_SYSCLK_SOURCE == IS_PLL) && (_PLL_SOURCE == IS_HSI))
#define __RCC_CFGR_VALUE                                                                                               \
    (RCC_PLLMUL_VAL(_PLLMUL) | RCC_PLLSRC_IS_HSI | RCC_ADCPRE_VAL(_ADCPRE) | RCC_CFGR_SW_HSI | RCC_PPRE1_VAL(_PPRE1) | \
     RCC_PPRE2_VAL(_PPRE2))
