...
namespace lcontrol{
	...
	void control::movement(int seqNum, int flag, float frontBackTilt, float leftRightTilt, float verticalSpeed, float angularSpeed, ClientSocket &s){
		...
		std::string seqStr = control::int_to_string(seqNum) ;
		data += seqStr + "," + control::int_to_string(flag) + "," + control::float_to_string(leftRightTilt) + "," + control::float_to_string(frontBackTilt) + "," + control::float_to_string(verticalSpeed) + "," + control::float_to_string(angularSpeed) + ",\r";
		std::cout << data << '\n';
		s.write(data) ;
	}
}
